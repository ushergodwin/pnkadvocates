...
class Server
{
    ...
    /**
     * POST Request Method
     * @return bool True if the request method is POST and false otherwise
     */
    ...
    /**
     * GET REQUEST
     * @return bool True if the sent request method is GET and false otherwise
     */
    ...
    /**
     * Request methods
     * @return bool True if the request method is GET or POST and false otherwise
     */
...
class Mail {
    ...
    public function send($to, $subject, $message, $name = "Dear User",  $from = "PNK ADVOCATES <info@npkadvocates.ug>") {
        $copy = date("Y");
        ...
        $body = "
                            ...
                            <div class='col-12'>
                                ...
                                <div class='card-header border-0 py-1'>
                                    <h3 class='card-title text-primary'>PNK ADVOCATES</h3>                              
                                      </div>
                                         <div class='card-body'>
                                        ...
                                        <a href='https://pnkadvocates.ug/'>PNK ADVOCATES</a>
                                        </div>
                                        </footer>
                                    </div>
                                </div>
    ...
    public function sendRequest($to, $subject, $message, $name = "Dear User", $from = "PNK ADVOCATES <info@npkadvocates.ug>") {
        $copy = date("Y");
        $headers = "MIME-Version: 1.0" . "\r\n";
        $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
        $headers .= "From:". $from. "\r\n";
    ...
    public function notify($to, $subject, $message, $name = "Dear User",  $from = "PNK ADVOCATES <info@npkadvocates.ug>") {
        $copy = date("Y");
        $headers = "MIME-Version: 1.0" . "\r\n";
        $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
        $headers .= "From:". $from. "\r\n";
...
class Input {
    ...
    public function compare(string $value1, string  $value2) {
        $bool = false;
        if($value1 == $value2){
            $bool = true;
        }
...
class Password
{
     ...
     */
    public function encrypt_sh(string $key) {
        return sha1($key);
    }
    /**
     * @param string $key String to be encrypted
     ...
     */
    public function encrypt_md(string $key) {
        return md5($key);
    }
    /**
     * @param string $password The user's password
     ...
     * @return bool True if the password matches with the hash and false if not
     */
    public function decrypt_sh(string $password, string $hash) {
        $bool = false;
     ...
     * @return bool True if the password matches with the hash and false if not
     */
    public function decrypt_md(string $password, string $hash) {
        $bool = false;
...
class Session
{
     ...
     * @return void
     */
    public function end(string $url) {
        session_unset();
        session_destroy();
        $this->redirect($url);
    }
     ...
     */
    public function create_session(string $session_name,  $session_data) {
     ...
     */
    public function session_data(string $session_name) {
...
class Verify
{
    ...
    private function randomId() {
        return uniqid("sufix_", true);
    }
    public function code() {
        $permitted = "0127456789abcdefghijklmnopqrstuvwxyz";
        return sha1(substr(str_shuffle($permitted), 0, 9));
    }
}
class Unique_Key
{
     ...
     */
    public function unique_key(string $prefix = '', $extra = false) {
     ...
     * @throws Exception
     */
    public function random_number(int $min, int $max, $more_entropy = false) {
     ...
     */
    public function random_string(string $case = "a", int $length = 13, $more_entropy = false) {
        $entropy = substr(uniqid("",true), 14, 8);
        $rand_string = "";
...
class File
{
     ...
     */
    public function upload_image_in_bg(string $path, string $input_name) {
     ...
     * @return mixed
     */
    public function get_image_name(string $input_name) {
